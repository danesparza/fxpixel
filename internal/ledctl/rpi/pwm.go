...
func (rp *RPi) InitPWM(freq uint, buf *DMABuf, bytes uint, pins []int) error {
	...
	if rp.pwmBuf == nil {
		...
		log.Debug().Msgf("Got pwmBuf[%d], offset %d\n", len(rp.pwmBuf), bufOffs)
		...
		log.Debug().Msgf("Got cmClkBuf[%d], offset %d\n", len(rp.cmClkBuf), bufOffs)
	...
	log.Debug().Msgf("Waiting for cmClk busy\n")
	i := 0
	for (rp.cmClk.ctl & CM_CLK_CTL_BUSY) == 0 {
		i++
	}
	log.Debug().Msgf("Done %d\n", i)
...
func (rp *RPi) StopPWM() {
	...
	time.Sleep(10 * time.Microsecond)
	log.Debug().Msgf("Waiting for cmClk not-busy\n")
	i := 0
	for (rp.cmClk.ctl & CM_CLK_CTL_BUSY) != 0 {
		i++
	}
