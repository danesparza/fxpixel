...
import (
	"fmt"
	pixarray "github.com/danesparza/fxpixel/internal/ledctl/pixarray"
...
// lcm includes every changing channel. Use int64 because the LCM of four
// 8-bit channel differences can overflow int on 32-bit Raspberry Pis.
func lcm(p pixarray.Pixel) int64 {
	result := int64(1)
	for _, channel := range []int{p.R, p.G, p.B, p.W} {
		if channel == 0 {
			continue
		}
		value := int64(abs(channel))
		a, b := result, value
		for b != 0 {
			a, b = b, a%b
		}
...
func (f *Fade) Start(pa *pixarray.PixArray, now time.Time) {
	log.Debug().Msgf("Starting Fade, dest %v", f.dest)
	...
	if maxdiff.R == 0 && maxdiff.G == 0 && maxdiff.B == 0 && maxdiff.W == 0 {
		...
		nsStep := f.fadeTime.Nanoseconds() / lcm(maxdiff)
		if f.allSame {
			log.Debug().Msgf("Starting all-same")
			nsStep = nsStep / int64(pa.NumPixels())
		}
		// Zero signals completion to callers, so keep unfinished fades positive.
		f.timeStep = maxDuration(time.Duration(nsStep), time.Nanosecond)
	}
	log.Debug().Msgf("Fade md.R %d, md.G %d, md.B %d, md.W %d, timestep %v", maxdiff.R, maxdiff.G, maxdiff.B, maxdiff.W, f.timeStep)
	f.start = now
}
...
func (f *Fade) NextStep(pa *pixarray.PixArray, now time.Time) time.Duration {
	...
	if f.allSame {
		...
		if nrp+ngp+nbp+nwp > 4.35 {
			log.Debug().Msgf("Weird, no diffs for r,g,b,w")
			return f.timeStep
		}
	...
	if f.allSame {
		log.Debug().Msgf("Setting all-same")
		f.timeStep = maxDuration(f.timeStep/time.Duration(pa.NumPixels()), time.Nanosecond)
	}
	return f.timeStep
}
...
func (r *Rainbow) Start(pa *pixarray.PixArray, now time.Time) {
	log.Debug().Msgf("Starting Rainbow")
	r.start = now
}
...
func (c *Cycle) Start(pa *pixarray.PixArray, now time.Time) {
	...
	case 0:
		...
		log.Debug().Msgf("Black->Red")
		c.last.R = 127
	case c.last.R:
		// Red max
		c.last.R = 127
		if c.last.G > c.last.B {
			log.Debug().Msgf("Red->Red+Green")
			c.last.B = 0
		} else {
			log.Debug().Msgf("Blue+Red->Red")
			c.last.G = 0
		}
	case c.last.G:
		// Green max
		...
		if c.last.B > c.last.R {
			log.Debug().Msgf("Green->Green+Blue")
			c.last.R = 0
		} else {
			log.Debug().Msgf("Red+Green->Green")
			c.last.B = 0
		}
	case c.last.B:
		// Blue max
		...
		if c.last.G > c.last.R {
			log.Debug().Msgf("Green+Blue->Blue")
			c.last.R = 0
		} else {
			log.Debug().Msgf("Blue->Blue+Red")
			c.last.G = 0
		}
	default:
	...
	if c.last != p {
		...
		log.Debug().Msgf("First fade to %v, max dist %d -> time %s", c.last, m, t)
		c.fade = NewFade(t, c.last)
		c.fade.Start(pa, now)
	} else {
		log.Debug().Msgf("Already in-cycle, no initial fade needed")
		c.NextStep(pa, now)
	}
}
...
func (kr *KnightRider) Start(pa *pixarray.PixArray, now time.Time) {
	log.Debug().Msgf("Starting KnightRider")
	kr.start = now
